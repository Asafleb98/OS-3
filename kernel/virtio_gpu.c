...
// The process whose buffer currently backs the display resource, or 0
// when the device is backed by the kernel framebuffer fb[].  Set by
// virtio_gpu_flip(); used by virtio_gpu_release() to restore fb[] when
// that process is torn down, so the device never scans out freed pages.
static struct proc *fb_owner;
//my CODE************************
...
// ════════════════════════════════════════════════════════════════════
//  Public helpers used by sys_map_display (Assignment 3, Task 1).
// ════════════════════════════════════════════════════════════════════

// Install the kernel framebuffer pages fb[] into `pagetable` at virtual
// address `va`, with PTE_U|PTE_R|PTE_W permission on every page.
//   - `va` must be page-aligned.
//   - The region [va, va + FB_PAGES*PGSIZE) must not collide with any
//     existing mapping.
//   - On failure (collision, out of memory in walk()), any partial
//     mapping is rolled back and -1 is returned.
...
virtio_gpu_map_user(pagetable_t pagetable, uint64 va)
{
    ...
    // Install the mapping page by page.  fb[i] is the kernel-owned
    // physical page that the GPU device is also reading from.
    for (int i = 0; i < FB_PAGES; i++) {
        if (mappages(pagetable, va + (uint64)i * PGSIZE, PGSIZE,
                     (uint64)fb[i], PTE_U | PTE_R | PTE_W) != 0) {
            // Roll back what we already installed.
            if (i > 0)
...
// Remove a previously installed framebuffer mapping from `pagetable`.
...
// ════════════════════════════════════════════════════════════════════
//  Public helper used by sys_flip_display (Assignment 3, Task 2).
// ════════════════════════════════════════════════════════════════════

// Re-point the GPU device's resource backing list at the FB_PAGES
// physical pages that back the user buffer at virtual address `va` in
// `pagetable`.  No pixel data is copied; only the (phys_addr, length)
// entries the device reads from are updated.
//
//   - `va` must be page-aligned.
//   - Every page in [va, va + FB_PAGES*PGSIZE) must be present in
//     `pagetable` with PTE_U|PTE_R|PTE_W permission.
//   - Returns 0 on success, -1 on any validation failure.
//
// We walk page-by-page (rather than passing `va` to the device) because
// the user buffer is virtually contiguous but the underlying physical
// pages — allocated one-by-one by sbrk()/uvmalloc() — almost certainly
// are NOT.  The device speaks physical addresses, so we hand it the
// per-page PA list explicitly.
//*************My code********************** */
int
virtio_gpu_flip(pagetable_t pagetable, uint64 va)
{
    // Static, page-by-page entries buffer.  Lives in BSS, not on the
    // 4KB kernel stack (FB_PAGES * 16B = 4800B would overflow it).
    // Two concurrent flippers would race over this buffer, but the same
    ...
    // Validate every page AND collect its physical address in one pass.
    // If any page fails validation, bail out before sending any GPU
    // command so the device's current backing is left intact.
    for (int i = 0; i < FB_PAGES; i++) {
        pte_t *pte = walk(pagetable, va + (uint64)i * PGSIZE, 0);
...
// Restore the kernel framebuffer fb[] as the device backing, but only if
// `p` is the process that currently owns it (set by virtio_gpu_flip).
